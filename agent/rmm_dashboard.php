<?php
require_once "includes/inc_all.php";
enforceUserPermission('module_rmm');

// Stats
$stats = mysqli_fetch_assoc(mysqli_query($mysqli,
    "SELECT
       SUM(rmm_status='online') as online,
       SUM(rmm_status='offline') as offline,
       SUM(rmm_status='unknown') as unknown,
       COUNT(*) as total
     FROM asset_rmm_links"
));

$alert_counts = mysqli_fetch_assoc(mysqli_query($mysqli,
    "SELECT
       SUM(status='new') as new_cnt,
       SUM(status='acknowledged') as ack_cnt
     FROM rmm_alerts"
));

$open_tickets = intval(mysqli_fetch_assoc(mysqli_query($mysqli,
    "SELECT COUNT(*) as c FROM tickets WHERE ticket_archived_at IS NULL AND ticket_resolved_at IS NULL"
))['c']);

// Offline assets
$sql_offline = mysqli_query($mysqli,
    "SELECT arl.*, a.asset_name, c.client_name, a.asset_client_id
     FROM asset_rmm_links arl
     JOIN assets a ON a.asset_id = arl.asset_id
     LEFT JOIN clients c ON c.client_id = a.asset_client_id
     WHERE arl.rmm_status='offline'
     ORDER BY arl.last_seen DESC
     LIMIT 20"
);

// Active alerts
$sql_new_alerts = mysqli_query($mysqli,
    "SELECT a.*, ast.asset_name, c.client_name
     FROM rmm_alerts a
     LEFT JOIN assets ast ON ast.asset_id = a.asset_id
     LEFT JOIN clients c ON c.client_id = a.client_id
     WHERE a.status='new'
     ORDER BY a.created_at DESC
     LIMIT 10"
);

// Recent remote sessions
$sql_recent_sessions = mysqli_query($mysqli,
    "SELECT rs.*, a.asset_name, u.user_firstname, u.user_lastname
     FROM rmm_remote_sessions rs
     JOIN assets a ON a.asset_id = rs.asset_id
     LEFT JOIN users u ON u.user_id = rs.user_id
     ORDER BY rs.created_at DESC
     LIMIT 10"
);

// Recent script runs
$sql_recent_runs = mysqli_query($mysqli,
    "SELECT sr.*, s.name as script_name, a.asset_name, u.user_firstname, u.user_lastname
     FROM rmm_script_runs sr
     LEFT JOIN rmm_scripts s ON s.id = sr.script_id
     JOIN assets a ON a.asset_id = sr.asset_id
     LEFT JOIN users u ON u.user_id = sr.user_id
     ORDER BY sr.started_at DESC
     LIMIT 10"
);

// Client health: clients with RMM assets
$sql_clients = mysqli_query($mysqli,
    "SELECT c.client_id, c.client_name,
       SUM(arl.rmm_status='online') as online,
       SUM(arl.rmm_status='offline') as offline,
       COUNT(arl.id) as total,
       (SELECT COUNT(*) FROM rmm_alerts al WHERE al.client_id = c.client_id AND al.status='new') as alerts,
       (SELECT COUNT(*) FROM tickets t WHERE t.ticket_client_id = c.client_id AND t.ticket_archived_at IS NULL AND t.ticket_resolved_at IS NULL) as open_tickets
     FROM clients c
     JOIN assets a ON a.asset_client_id = c.client_id
     JOIN asset_rmm_links arl ON arl.asset_id = a.asset_id
     WHERE c.client_archived_at IS NULL
     GROUP BY c.client_id
     HAVING total > 0
     ORDER BY offline DESC, alerts DESC, c.client_name ASC"
);
?>

<div class="d-flex align-items-center mb-3">
    <h4 class="mb-0 mr-auto"><i class="fas fa-tachometer-alt mr-2"></i>RMM Dashboard</h4>
    <a href="/agent/rmm_assets.php" class="btn btn-info btn-sm mr-2">
        <i class="fas fa-desktop mr-1"></i>All Assets
    </a>
    <?php if (lookupUserPermission('module_rmm_sync') >= 1): ?>
    <button class="btn btn-success btn-sm" onclick="quickSync()">
        <i class="fas fa-sync mr-1"></i>Sync Now
    </button>
    <?php endif; ?>
</div>

<!-- Top-level stat cards -->
<div class="row mb-3">
    <div class="col-md-2">
        <div class="info-box bg-success"><span class="info-box-icon"><i class="fas fa-circle"></i></span>
            <div class="info-box-content"><span class="info-box-text">Online</span><span class="info-box-number"><?= intval($stats['online']) ?></span></div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="info-box bg-danger"><span class="info-box-icon"><i class="fas fa-circle"></i></span>
            <div class="info-box-content"><span class="info-box-text">Offline</span><span class="info-box-number"><?= intval($stats['offline']) ?></span></div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="info-box bg-secondary"><span class="info-box-icon"><i class="fas fa-desktop"></i></span>
            <div class="info-box-content"><span class="info-box-text">Total Managed</span><span class="info-box-number"><?= intval($stats['total']) ?></span></div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="info-box bg-warning"><span class="info-box-icon"><i class="fas fa-bell"></i></span>
            <div class="info-box-content"><span class="info-box-text">New Alerts</span><span class="info-box-number"><?= intval($alert_counts['new_cnt']) ?></span></div>
        </div>
    </div>
    <div class="col-md-2">
        <div class="info-box bg-primary"><span class="info-box-icon"><i class="fas fa-ticket-alt"></i></span>
            <div class="info-box-content"><span class="info-box-text">Open Tickets</span><span class="info-box-number"><?= $open_tickets ?></span></div>
        </div>
    </div>
</div>

<div id="syncBar" class="alert alert-info d-none mb-3">
    <i class="fas fa-spinner fa-spin mr-2"></i><span id="syncBarText">Syncing...</span>
</div>

<div class="row">
    <!-- Left column -->
    <div class="col-md-6">

        <!-- Offline Assets -->
        <div class="card card-dark mb-3" style="border-top:3px solid #dc3545">
            <div class="card-header py-2 d-flex align-items-center">
                <h6 class="mb-0 mr-auto"><i class="fas fa-times-circle mr-2 text-danger"></i>Offline Assets</h6>
                <a href="/agent/rmm_assets.php?status=offline" class="btn btn-xs btn-secondary">All</a>
            </div>
            <div class="card-body p-0">
            <?php if (mysqli_num_rows($sql_offline) === 0): ?>
                <p class="text-muted text-center py-3 mb-0 small"><i class="fas fa-check-circle text-success mr-1"></i>All assets online</p>
            <?php else: ?>
                <table class="table table-sm table-hover mb-0">
                <?php while ($row = mysqli_fetch_assoc($sql_offline)):
                    $rmm_lnk = mysqli_fetch_assoc(mysqli_query($mysqli, "SELECT id FROM asset_rmm_links WHERE asset_id=" . intval($row['asset_id']) . " LIMIT 1"));
                ?>
                    <tr>
                        <td class="pl-3 small">
                            <a href="/agent/rmm_asset.php?id=<?= intval($row['id']) ?>">
                                <?= nullable_htmlentities($row['hostname'] ?: $row['asset_name']) ?>
                            </a>
                        </td>
                        <td class="small text-muted"><?= nullable_htmlentities($row['client_name']) ?></td>
                        <td class="small text-muted"><?= nullable_htmlentities($row['last_seen']) ?></td>
                    </tr>
                <?php endwhile; ?>
                </table>
            <?php endif; ?>
            </div>
        </div>

        <!-- New Alerts -->
        <div class="card card-dark mb-3" style="border-top:3px solid #ffc107">
            <div class="card-header py-2 d-flex align-items-center">
                <h6 class="mb-0 mr-auto"><i class="fas fa-bell mr-2 text-warning"></i>New Alerts</h6>
                <a href="/agent/rmm_alerts.php" class="btn btn-xs btn-secondary">All Alerts</a>
            </div>
            <div class="card-body p-0">
            <?php if (mysqli_num_rows($sql_new_alerts) === 0): ?>
                <p class="text-muted text-center py-3 mb-0 small"><i class="fas fa-check-circle text-success mr-1"></i>No new alerts</p>
            <?php else: ?>
                <?php while ($alert = mysqli_fetch_assoc($sql_new_alerts)):
                    $sev_color = ['critical'=>'danger','error'=>'danger','warning'=>'warning','info'=>'info'][$alert['severity']] ?? 'secondary';
                ?>
                <div class="d-flex align-items-center border-bottom px-3 py-2 small">
                    <span class="badge badge-<?= $sev_color ?> mr-2"><?= nullable_htmlentities($alert['severity']) ?></span>
                    <span class="mr-auto"><?= nullable_htmlentities(substr($alert['message'], 0, 80)) ?></span>
                    <span class="text-muted ml-2"><?= nullable_htmlentities($alert['client_name']) ?></span>
                </div>
                <?php endwhile; ?>
            <?php endif; ?>
            </div>
        </div>

    </div>

    <!-- Right column -->
    <div class="col-md-6">

        <!-- Client Health -->
        <div class="card card-dark mb-3">
            <div class="card-header py-2">
                <h6 class="mb-0"><i class="fas fa-users mr-2"></i>Client Health</h6>
            </div>
            <div class="card-body p-0">
            <?php if (mysqli_num_rows($sql_clients) === 0): ?>
                <p class="text-muted text-center py-3 mb-0 small">No clients with RMM assets.</p>
            <?php else: ?>
                <table class="table table-sm table-hover mb-0">
                    <thead class="text-muted small border-bottom" style="font-size:11px;text-transform:uppercase;letter-spacing:.4px;">
                        <tr>
                            <th class="pl-3">Client</th>
                            <th class="text-center">Assets</th>
                            <th class="text-center text-danger">Offline</th>
                            <th class="text-center text-warning">Alerts</th>
                            <th class="text-center">Tickets</th>
                        </tr>
                    </thead>
                    <tbody>
                    <?php while ($cl = mysqli_fetch_assoc($sql_clients)): ?>
                    <tr class="<?= ($cl['offline'] > 0 || $cl['alerts'] > 0) ? 'table-warning' : '' ?>">
                        <td class="pl-3 small">
                            <a href="/agent/client_details.php?client_id=<?= intval($cl['client_id']) ?>">
                                <?= nullable_htmlentities($cl['client_name']) ?>
                            </a>
                        </td>
                        <td class="text-center small"><?= intval($cl['total']) ?></td>
                        <td class="text-center small <?= $cl['offline'] > 0 ? 'text-danger font-weight-bold' : 'text-muted' ?>">
                            <?= intval($cl['offline']) ?>
                        </td>
                        <td class="text-center small <?= $cl['alerts'] > 0 ? 'text-warning font-weight-bold' : 'text-muted' ?>">
                            <?= intval($cl['alerts']) ?>
                        </td>
                        <td class="text-center small text-muted"><?= intval($cl['open_tickets']) ?></td>
                    </tr>
                    <?php endwhile; ?>
                    </tbody>
                </table>
            <?php endif; ?>
            </div>
        </div>

        <!-- Recent Remote Sessions -->
        <div class="card card-dark mb-3">
            <div class="card-header py-2">
                <h6 class="mb-0"><i class="fas fa-desktop mr-2"></i>Recent Connections</h6>
            </div>
            <div class="card-body p-0">
            <?php if (mysqli_num_rows($sql_recent_sessions) === 0): ?>
                <p class="text-muted text-center py-3 mb-0 small">No remote sessions logged.</p>
            <?php else: ?>
                <?php while ($sess = mysqli_fetch_assoc($sql_recent_sessions)): ?>
                <div class="d-flex border-bottom px-3 py-2 small align-items-center">
                    <i class="fas fa-desktop mr-2 text-info"></i>
                    <a href="/agent/asset_details.php?asset_id=<?= intval($sess['asset_id']) ?>" class="mr-auto">
                        <?= nullable_htmlentities($sess['asset_name']) ?>
                    </a>
                    <span class="text-muted"><?= nullable_htmlentities($sess['user_firstname'] . ' ' . $sess['user_lastname']) ?></span>
                    <span class="text-muted ml-3"><?= nullable_htmlentities($sess['created_at']) ?></span>
                </div>
                <?php endwhile; ?>
            <?php endif; ?>
            </div>
        </div>

        <!-- Recent Script Runs -->
        <div class="card card-dark mb-3">
            <div class="card-header py-2">
                <h6 class="mb-0"><i class="fas fa-code mr-2"></i>Recent Script Runs</h6>
            </div>
            <div class="card-body p-0">
            <?php if (mysqli_num_rows($sql_recent_runs) === 0): ?>
                <p class="text-muted text-center py-3 mb-0 small">No script runs yet.</p>
            <?php else: ?>
                <?php while ($run = mysqli_fetch_assoc($sql_recent_runs)):
                    $run_badge = ['completed'=>'success','failed'=>'danger','running'=>'warning','pending'=>'secondary'][$run['status']] ?? 'secondary';
                ?>
                <div class="d-flex border-bottom px-3 py-2 small align-items-center">
                    <span class="badge badge-<?= $run_badge ?> mr-2"><?= $run['status'] ?></span>
                    <span class="mr-auto font-weight-bold"><?= nullable_htmlentities($run['script_name'] ?? 'Manual') ?></span>
                    <span class="text-muted"><?= nullable_htmlentities($run['asset_name']) ?></span>
                    <span class="text-muted ml-3"><?= nullable_htmlentities($run['started_at']) ?></span>
                </div>
                <?php endwhile; ?>
            <?php endif; ?>
            </div>
        </div>

    </div>
</div>

<script>
function quickSync() {
    const bar = document.getElementById('syncBar');
    const txt = document.getElementById('syncBarText');
    bar.classList.remove('d-none');
    txt.textContent = 'Syncing assets from Tactical RMM...';
    fetch('/agent/post/rmm_sync.php', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded'},
        body: 'csrf_token=<?= $_SESSION['csrf_token'] ?>&action=sync&integration_id=<?= $config_rmm_default_integration_id ?>'
    })
    .then(r => r.json())
    .then(d => {
        if (d.success) {
            txt.textContent = `Sync complete: ${d.created} created, ${d.updated} updated, ${d.matched} matched.`;
            bar.classList.replace('alert-info', 'alert-success');
            setTimeout(() => location.reload(), 2000);
        } else {
            txt.textContent = 'Sync failed: ' + (d.error || 'Unknown');
            bar.classList.replace('alert-info', 'alert-danger');
        }
    });
}
</script>

<?php require_once "../includes/footer.php"; ?>
