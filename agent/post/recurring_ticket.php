    ...
    if (isset($_POST['recurring_ticket_ids'])) {
        ...
        foreach ($_POST['recurring_ticket_ids'] as $recurring_ticket_id) {
            ...
            if (mysqli_num_rows($sql) > 0) {
                ...
                if ((!empty($config_smtp_host) || !empty($config_smtp_provider)) && $config_ticket_client_general_notifications == 1 && filter_var($contact_email, FILTER_VALIDATE_EMAIL)) {
...
if (isset($_POST['bulk_assign_recurring_ticket'])) {
    ...
    if (!empty($_POST['recurring_ticket_ids'])) {
        ...
        if ($session_user_id != $assign_to && $assign_to != 0) {
            ...
            // App Notification
            notifyUser($assign_to, 'Recurring Ticket', "$recurring_ticket_count Recurring Tickets have been assigned to you by $session_name", "recurring_tickets.php?assigned=$assign_to", $client_id);
