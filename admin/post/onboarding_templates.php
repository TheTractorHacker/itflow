...
if (isset($_POST['add_onboarding_template'])) {
    ...
    mysqli_query($mysqli, "INSERT INTO project_templates SET
        ...
        project_template_is_onboarding = 1");

    $project_template_id = mysqli_insert_id($mysqli);

    // Create the companion checklist ticket template
    mysqli_query($mysqli, "INSERT INTO ticket_templates SET
        ticket_template_name = '$name Checklist',
        ticket_template_subject = '$name',
        ticket_template_description = '$description'");

    $ticket_template_id = mysqli_insert_id($mysqli);

    mysqli_query($mysqli, "INSERT INTO project_template_ticket_templates SET
        project_template_id = $project_template_id,
        ticket_template_id = $ticket_template_id,
        ticket_template_order = 0");

    logAction("Onboarding Template", "Create", "$session_name created onboarding template $name", 0, $project_template_id);

    flash_alert("Onboarding Template <strong>$name</strong> created");

    redirect("onboarding_template_details.php?project_template_id=$project_template_id");
