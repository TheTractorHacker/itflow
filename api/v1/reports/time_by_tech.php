...
$sql_assigned = mysqli_query($mysqli,
    "SELECT ticket_assigned_to AS uid, COUNT(ticket_id) AS c
     FROM tickets
     WHERE YEAR(ticket_created_at) = $year AND ticket_assigned_to > 0
...
$sql_touched = mysqli_query($mysqli,
    "SELECT uid, COUNT(DISTINCT ticket_id) AS c FROM (
        ...
        FROM ticket_replies WHERE YEAR(ticket_reply_created_at) = $year
        UNION
        SELECT ticket_created_by AS uid, ticket_id FROM tickets WHERE YEAR(ticket_created_at) = $year
        UNION
        SELECT ticket_closed_by AS uid, ticket_id FROM tickets
        WHERE YEAR(ticket_created_at) = $year AND ticket_closed_by IS NOT NULL
     ) touched
     GROUP BY uid"
);
...
$sql_time = mysqli_query($mysqli,
    "SELECT tr.ticket_reply_by AS uid, SUM(TIME_TO_SEC(tr.ticket_reply_time_worked)) AS secs
     ...
     WHERE YEAR(t.ticket_created_at) = $year AND tr.ticket_reply_time_worked IS NOT NULL
