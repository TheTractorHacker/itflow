...
// FCM token update (needs bearer auth)
if ($method === 'POST' && !empty($segments[1]) && $segments[1] === 'fcm') {
    $bearer = '';
    if (preg_match('/^Bearer\s+(\S+)$/i', $_SERVER['HTTP_AUTHORIZATION'] ?? '', $bm)) {
        $bearer = $bm[1];
    }
    if (!$bearer) api_error(401, 'Unauthorized');
    $hash  = mysqli_real_escape_string($mysqli, hash('sha256', $bearer));
...
$sql      = mysqli_query($mysqli,
    "SELECT user_id, user_name, user_email, user_type, user_password, user_specific_encryption_ciphertext
     FROM users
...
// Derive master encryption key from password (same as web login)
...
// Encrypt master key with a key derived from the raw token (AES-128-CBC)
